...
class PersonnelUpdateImport implements SkipsUnknownSheets, ToCollection, WithMultipleSheets, WithStartRow
{
    ...
    public function generatePreview(Collection $rows): array
    {
        ...
        foreach ($rows as $rowIndex => $row) {
            ...
            if (empty($rankInput) && ! is_numeric($no)) continue;
            $nameLower = strtolower($fullName);
            if (str_starts_with($fullName, '=') || in_array($nameLower, ['jumlah', 'total'])) continue;
            ...
            foreach ($rows as $idx => $data) {
                ...
                try {
                    if ($data['action'] === 'update' && !empty($data['personnel_id'])) {
                        ...
                        if (!empty($data['bagian']))     $updateData['bagian']     = $data['bagian'];
                        if (!empty($data['golongan']))   $updateData['golongan']   = $data['golongan'];
                        if (!empty($data['gender']))     $updateData['gender']     = $data['gender'];
                        if ($data['keterangan'] !== '')  $updateData['keterangan'] = $data['keterangan'];
                        if (!empty($data['rank_id']))    $updateData['rank_id']    = $data['rank_id'];
                        ...
                        if (!empty($merged)) $updateData['kapor_sizes'] = $merged;
