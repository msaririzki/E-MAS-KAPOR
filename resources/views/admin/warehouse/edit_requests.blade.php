...
<div class="content-header">
    <div class="header-left">
        <h1 class="page-title">Pengajuan Data Gudang</h1>
        <p class="page-subtitle">Kelola pengajuan Edit Stok dan Pengeluaran dari Admin Gudang</p>
    </div>
</div>
                                ...
                                <td>
                                    @if($req->status === 'pending')
                                        <div style="display:flex; gap:8px;">
                                            <form action="{{ route('admin.warehouse-items.edit-requests.approve', $req->id) }}" method="POST" onsubmit="confirmAction(event, 'MENYETUJUI', 'edit')">
                                                @csrf
                                                <button type="submit" class="btn-icon" style="background:#10B981; color:white; border:none; width:34px; height:34px; border-radius:6px; cursor:pointer;" title="Setujui">
                                                    <i class="ri-check-line"></i>
                                                </button>
                                            </form>
                                            <form action="{{ route('admin.warehouse-items.edit-requests.reject', $req->id) }}" method="POST" onsubmit="confirmAction(event, 'MENOLAK', 'edit')">
                                                @csrf
                                                <button type="submit" class="btn-icon" style="background:#EF4444; color:white; border:none; width:34px; height:34px; border-radius:6px; cursor:pointer;" title="Tolak">
                        ...
                        @forelse($dispenseRequests as $dReq)
                            <tr>
                                ...
                                <td>
                                    @if($dReq->status === 'pending')
                                        <div style="display:flex; gap:8px;">
                                            <form action="{{ route('admin.warehouse-items.dispense-requests.approve', $dReq->id) }}" method="POST" onsubmit="confirmAction(event, 'MENYETUJUI', 'dispense')">
                                                @csrf
                                                <button type="submit" class="btn-icon" style="background:#10B981; color:white; border:none; width:34px; height:34px; border-radius:6px; cursor:pointer;" title="Setujui">
                                                    <i class="ri-check-line"></i>
                                                </button>
                                            </form>
                                            <form action="{{ route('admin.warehouse-items.dispense-requests.reject', $dReq->id) }}" method="POST" id="form_reject_dispense_{{ $dReq->id }}">
                                                @csrf
                                                <input type="hidden" name="reason" id="reason_dispense_{{ $dReq->id }}">
                                                <button type="button" onclick="rejectDispense('{{ $dReq->id }}')" class="btn-icon" style="background:#EF4444; color:white; border:none; width:34px; height:34px; border-radius:6px; cursor:pointer;" title="Tolak">
                                                    <i class="ri-close-line"></i>
                                                </button>
                                            </form>
                                        </div>
