...
@if($preview['missing_rows'] !== [])
    <div id="missingReviewModal" class="missing-review-modal" aria-hidden="true">
...
function openMissingReviewModal() {
    missingReviewModal?.classList.add('open');
    missingReviewModal?.setAttribute('aria-hidden', 'false');
}

function closeMissingReviewModal(scrollToList = false) {
    missingReviewModal?.classList.remove('open');
    missingReviewModal?.setAttribute('aria-hidden', 'true');
    if (scrollToList) {
        document.querySelector('.missing-section')?.scrollIntoView({behavior: 'smooth', block: 'center'});
    }
}
