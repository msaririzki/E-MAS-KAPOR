...
<form action="{{ route('admin.personnel.transfer-requests.review') }}" method="POST" id="transferReviewForm">
    ...
    @if($status === 'pending' && $requests->count() > 0)
        <div class="transfer-toolbar">
            <label><input type="checkbox" id="selectAllTransfers"> Pilih semua di halaman ini</label>
            <div>
                <input type="text" name="review_note" placeholder="Catatan pemeriksaan (opsional)">
                <button type="submit" name="action" value="reject" class="reject" onclick="return confirm('Yakin ingin menolak pengajuan yang dipilih?');"><i class="ri-close-line"></i>Tolak Terpilih</button>
                <button type="submit" name="action" value="approve" class="approve" onclick="return confirm('Setujui pengajuan yang dipilih?');"><i class="ri-check-line"></i>Setujui Terpilih</button>
                <button type="submit" name="action" value="approve_all" formaction="{{ route('admin.personnel.transfer-requests.approve-all') }}" class="approve-all" onclick="return confirm('Setujui SEMUA (seluruh data pending) pengajuan mutasi yang sedang menunggu?');"><i class="ri-check-double-line"></i>Setujui Semua Pending</button>
            </div>
        </div>
    @elseif($status === 'pending' && ($counts['pending'] ?? 0) > 0)
        <div class="transfer-toolbar">
            <div></div>
            <div>
                <button type="submit" name="action" value="approve_all" formaction="{{ route('admin.personnel.transfer-requests.approve-all') }}" class="approve-all" onclick="return confirm('Setujui SEMUA (seluruh data pending) pengajuan mutasi yang sedang menunggu?');"><i class="ri-check-double-line"></i>Setujui Semua Pending</button>
    ...
    <div class="transfer-table-shell">
        <div class="transfer-scroll">
            <table>
                <thead>
                    <tr>
                        @if($status === 'pending')<th style="width: 40px; text-align: center;"></th>@endif
                        <th>PERSONEL</th>
                        <th>SATKER ASAL</th>
                        <th>SATKER TUJUAN</th>
                        <th>DATA DIAJUKAN</th>
                ...
                <tbody>
                        ...
                        <tr>
                            @if($status === 'pending')
                                <td style="text-align: center;">
                                    <input type="checkbox" name="request_ids[]" value="{{ $transfer->id }}" class="transfer-checkbox">
                                </td>
                            @endif
                    ...
                    @empty
                        <tr><td colspan="{{ $status === 'pending' ? 7 : 6 }}"><div class="transfer-empty"><i class="ri-inbox-2-line"></i><strong>Tidak ada data mutasi pada status ini</strong></div></td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
...
.transfer-alert.info{background:#EFF6FF;border-color:#BFDBFE;color:#1E40AF}
.transfer-tabs{display:flex;border-bottom:1px solid #E5E7EB;margin-bottom:14px}
.transfer-tabs a{height:42px;padding:0 16px;color:#64748B;text-decoration:none;display:inline-flex;align-items:center;gap:7px;font-size:12px;font-weight:700;border-bottom:2px solid transparent}
.transfer-tabs a.active{color:#B91C1C;border-bottom-color:#B91C1C}
.transfer-tabs strong{min-width:21px;height:21px;border-radius:999px;background:#F1F5F9;color:#475569;display:inline-flex;align-items:center;justify-content:center;font-size:9px}
.transfer-toolbar{padding:11px 14px;border:1px solid #E5E7EB;border-radius:8px 8px 0 0;background:#F8FAFC;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px}
.transfer-toolbar>label{font-size:11px;color:#475569;font-weight:700;display:inline-flex;align-items:center;gap:6px;cursor:pointer}
.transfer-toolbar>div{display:flex;align-items:center;gap:7px;flex-wrap:wrap}
.transfer-toolbar input[type=text]{height:36px;width:240px;border:1px solid #CBD5E1;border-radius:7px;padding:0 10px;font-size:11px}
.transfer-toolbar button{height:36px;border-radius:7px;border:1px solid;display:inline-flex;align-items:center;gap:5px;padding:0 12px;font-size:11px;font-weight:800;cursor:pointer}
.transfer-toolbar button.reject{background:#fff;border-color:#FCA5A5;color:#B91C1C}
.transfer-toolbar button.approve{background:#047857;border-color:#047857;color:#fff}
.transfer-toolbar button.approve-all{background:#1E3A8A;border-color:#1E3A8A;color:#fff}
.transfer-table-shell{border:1px solid #E5E7EB;border-radius:8px;background:#fff;overflow:hidden}
.transfer-toolbar+.transfer-table-shell{border-top:0;border-radius:0 0 8px 8px}
.transfer-scroll{overflow:auto}
.transfer-table-shell table{width:100%;min-width:1100px;border-collapse:collapse}
.transfer-table-shell th{height:40px;padding:0 11px;background:#F8FAFC;border-bottom:1px solid #E5E7EB;text-align:left;font-size:9px;color:#64748B}
.transfer-table-shell td{padding:12px 11px;border-bottom:1px solid #F1F5F9;vertical-align:top;font-size:11px;color:#334155}
.transfer-person,.payload-cell{display:flex;flex-direction:column;gap:3px}
.transfer-person strong,.payload-cell strong{font-size:11px;color:#111827}
.transfer-person code{font-size:10px;background:transparent;padding:0;color:#475569}
.transfer-person small,.payload-cell span,.payload-cell small{font-size:9px;color:#64748B}
.satker-label{display:inline-flex;padding:4px 7px;border-radius:5px;font-size:9px;font-weight:800}
.satker-label.from{background:#F1F5F9;color:#475569}
.satker-label.to{background:#ECFDF5;color:#047857}
.transfer-status{display:inline-flex;padding:4px 7px;border-radius:999px;font-size:9px;font-weight:800}
.transfer-status.pending{background:#FFF7ED;color:#C2410C}
.transfer-status.approved{background:#ECFDF5;color:#047857}
.transfer-status.rejected{background:#FEF2F2;color:#B91C1C}
.transfer-empty{padding:40px;display:flex;align-items:center;justify-content:center;flex-direction:column;gap:5px;color:#64748B}
...
@media(max-width:760px){
    ...
    .transfer-toolbar input[type=text]{width:100%}
}
</style>
@endsection
...
document.getElementById('transferReviewForm')?.addEventListener('submit', function (event) {
    if (event.submitter?.value === 'approve_all') {
        return;
    }

    if (document.querySelectorAll('.transfer-checkbox:checked').length === 0) {
        event.preventDefault();
        alert('Silakan pilih setidaknya satu pengajuan mutasi.');
    }
});
</script>
@endsection
