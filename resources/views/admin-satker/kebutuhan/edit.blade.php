...
<form method="POST" action="{{ route('admin-satker.kebutuhan.update', $kebutuhan) }}">
    ...
    <div class="card">
        <div class="card-head">
            ...
            <button type="button" class="btn btn-primary btn-sm" onclick="addItem()"><i class="ri-add-line"></i> Tambah Item</button>
        </div>
        <div class="card-body" id="itemsContainer">
            <div id="noItemsMessage" style="text-align: center; padding: 32px; color: var(--text-muted); display: none;">
...
<script>
    ...
    function removeItem(btn) {
        ...
        if (!document.getElementById('itemsContainer').querySelector('.item-row')) {
            document.getElementById('noItemsMessage').style.display = 'block';
        }
    }

    // Pre-populate existing items
    const existingItems = @json($kebutuhan->items->map(fn ($i) => ['kapor_item_id' => $i->kapor_item_id, 'quantity' => $i->quantity, 'notes' => $i->notes]));
